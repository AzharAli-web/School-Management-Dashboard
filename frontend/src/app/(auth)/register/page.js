...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    const assignedRole = await register(name, email, password, role);
    if (assignedRole) {
        ...
        <Card className="border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/50 dark:shadow-black/50 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl">
          <CardHeader className="space-y-3 text-center pb-6">
            ...
            <CardDescription className="text-zinc-500 dark:text-zinc-400">
              Join the School Management Dashboard
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter className="flex justify-center text-sm text-zinc-500 dark:text-zinc-400 pt-4 border-t border-zinc-100 dark:border-zinc-800/50 mt-2">
            Already have an account? <Link href="/login" className="text-primary hover:underline ml-1 font-medium">Sign In</Link>
          </CardFooter>
        </Card>
      </motion.div>
    </div>
